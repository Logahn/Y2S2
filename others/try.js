    ...
    function() {
        ...
        */
        ...
        if (isSafari || iOS) {
            ...
            $('#board1').text("Safari Browser or iOS does not support demo");
            return
        }
        ...
        */
        ...
        function game_result(result) {
            if (!game_over && !engine_analyzing) {
                $('#game_result_dis').html(result);
        ...
        */
        function display_clock(color, t) {
            ...
            if (t === 0){
                let rlt = "";
                if(game.in_draw()) {
                    rlt = "Time-up! Game is draw!";
                }else{
                    rlt = (game.turn() === "w"? "White": "Black") + " lost on time!";
                }
                return game_result(rlt);
            }
        ...
        function ChessAI() {
            if(!game_over && ((black_player === computer_player && game.turn() === 'b') ||
                (white_player === computer_player && game.turn() === 'w') ||
                (gettingHint))){
                /*let move = game.search({verbose: true});
                if(move === null) return;*/
                set_uci_board();
                raccoon_engine.postMessage('go depth ' + Math.floor(computer_level*1.2) +
                    ' wtime ' + time.wtime + ' winc ' + time.winc + ' btime ' + time.btime + ' binc ' + time.binc);
            }
        }
        ...
        */
        ...
        function onDragStart (source, piece) {
            if (!engine_analyzing && (game.game_over() || game_over)) return false;
        ...
        function onSnapEnd () {
            if(!solvingpuzzleMode && !game.game_over()){
                stop_clock();
                update_clock();
                start_clock();
            }
        ...
        enable_slide('.horizontal_scroll');
        enable_slide('.horizontal_scroll_pv1');
        enable_slide('.horizontal_scroll_pv2');
        ...
        $('#close_game_result').click(function () {
            $("#game_result").fadeOut();
        });
